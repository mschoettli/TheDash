...
router.get("/export", (_req, res) => {
  ...
  res.json({ settings, tiles, sections, links, notes });
});

router.post("/import", (req, res) => {
  const { settings, tiles, sections, links, notes } = req.body;
  ...
  const doImport = db.transaction(() => {
    ...
    for (const t of tiles ?? []) {
      db.prepare(
        "INSERT INTO tiles (id, name, url, icon_url, style, api_endpoint, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
      ).run(t.id, t.name, t.url, t.icon_url, t.style, t.api_endpoint, t.sort_order, t.created_at);
    }
    for (const s of sections ?? []) {
      db.prepare(
        "INSERT INTO sections (id, title, sort_order) VALUES (?, ?, ?)"
      ).run(s.id, s.title, s.sort_order);
    }
