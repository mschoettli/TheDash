...
export default function TileMetric({ tile, status, apiData }: Props) {
  ...
  return (
    <div className="tile-glass group relative flex h-full flex-col overflow-hidden rounded-xl border border-line/60 p-3 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-accent/35 hover:shadow-xl hover:shadow-accent/10">
      ...
      <div className="relative flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-line/55 bg-surface shadow-inner shadow-white/5">
            <FaviconImg url={tile.url} name={tile.name} size={23} explicitIconUrl={tile.icon_url} />
          </div>
          <div className="min-w-0">
            <div className="truncate text-[13px] font-bold text-t1">{tile.name}</div>
            {tile.show_address && (
      ...
      {metricItems.length > 0 ? (
        <div className="relative mt-3 grid grid-cols-2 gap-1.5">
          {metricItems.map(({ label, val }) => (
            <div key={label} className="rounded-xl border border-line/45 bg-card/60 px-2.5 py-2">
              <div className="mb-0.5 text-[8px] font-semibold uppercase tracking-[0.14em] text-t3">{label}</div>
              <div className="text-[15px] font-bold tabular-nums text-t1">{val ?? "–"}</div>
            </div>
          ))}
        </div>
